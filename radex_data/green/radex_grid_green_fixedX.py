...
# not used in this file orthopararatio = [0.0,0.1,1.0,3.0]
...
temperatures = [20,50,80] #[ tmin + (ii) / float(ntemp-1) * (tmax-tmin)  for ii in range(ntemp) ]
...
#mole = 'o-h2co-h2_faure'  # molecular data name
#/Users/adam/repos/radex/data/o-h2co-scaled1.6.dat
mole = 'o-h2co-scaled1.6'

# Naming suffix to append to line name defined in "acts" below
suffix = "_XH2CO_fixed_green"
...
for iact,act in enumerate(acts):
    ...
    for temp in temperatures:
        for dens,col in zip(densities,columns):
            ...
            if (temp == temperatures[-1] and
                    dens == densities[-1] and
                    col == columns[-1]):
                infile.write('0\n')
                infile.close()
            else:
